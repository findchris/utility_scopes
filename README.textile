...
The @use@ scope let's you tap into named_scope's powerful chaining capabilities by creating a scope for an individual ActiveRecord instance.  This has proven useful especially when combined with the @with@ scope for reloading a record with eager associations:
